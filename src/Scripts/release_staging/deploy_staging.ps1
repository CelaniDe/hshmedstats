Write-Warning "Starting staging release to azure"

$date = Get-Date
$backupName = "pcs_staging_"+$date.ToShortDateString()+".bacpac"
...
& 'C:\Program Files\sqlpackage\sqlpackage.exe' /a:export /tf:$backupPath /scs:'Data Source=pcsinvoicing.database.windows.net;Initial Catalog=pcsinvoicingdb;User Id=pcsadmin;Password=123*789adminPcs#$59147;'
...
Write-Warning "Transforming connection strings to staging..."
Invoke-Expression -Command "$PSScriptRoot\..\staging_transform.ps1"
...
if($res.ToLower() -eq "y"){
    Write-Warning "Deploying web client and web jobs to staging..."
    $password = Get-PasswordFromPublishProfile -publishProfilePath "$PSScriptRoot\pcs-staging.PublishSettings"
    if($password -ne ""){
        & $msbuildPath "$PSScriptRoot\..\..\PCS.sln" /p:DeployOnBuild=true /p:PublishProfile="pcs-staging - Web Deploy" /p:Configuration=Release /p:Password=$password  
    }
    Write-Warning "Finished app deployment to staging"
}
